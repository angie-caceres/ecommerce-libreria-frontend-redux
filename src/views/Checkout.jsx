// VISTA — página de checkout con Redux
...
function Checkout() {
  ...
  const handleNumeroTarjeta = (e) => {
    ...
    setTarjeta({
      ...tarjeta,
      numero: solo.replace(/(.{4})/g, "$1 ").trim(),
    });
  };
  ...
  const handleVencimiento = (e) => {
    ...
    setTarjeta({
      ...tarjeta,
      vencimiento:
        solo.length > 2 ? solo.slice(0, 2) + " / " + solo.slice(2) : solo,
    });
  };
  ...
  const handleConfirmarCompra = async () => {
    ...
    const metodo =
      metodoPago === "tarjeta" ? "TARJETA_CREDITO" : "MERCADO_PAGO";
    ...
    if (confirmarCompra.fulfilled.match(resultado)) {
      ...
      setErrorPago("No se pudo confirmar la compra.");
    }
  };
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDireccionTemp({
                          ...direccionTemp,
                          codigoPostal: e.target.value.slice(0, 8),
                        })
                      }
                      className="w-full border border-gray-300 p-3 mt-2 bg-white"
                    />
